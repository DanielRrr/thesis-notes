...
Let $\mathcal{N} = \langle E, V, l \rangle$ be a term prenetwork, $\mathcal{A}$ be a join-semilattice ordered semigroup and $\vartheta : V \to \underline{{\mathcal{A}}}$ a valuation. Let us define the prenetwork $\mathcal{N}^{\vartheta}$ with the same edges and vertices with labelling $l^{\vartheta}(x, y) = \{ t^{\vartheta} \: | \: t \in l_{\mathcal{N}}(x, y)\}$.
...
\begin{proof}
  ...
  Let $\mathcal{N}$ be a term network. We define the following three extensions of $\mathcal{N}$ that reflect the composition, witness, and join moves respectively:
  \begin{enumerate}
    \item $\mathcal{N}_c = $
    \item $\mathcal{N}_w = $
    \item $\mathcal{N}_j = $
...
This formula merely claims that $\exists$ has a winning strategy in the zero length game.
    ...
    \item $\sigma_{n + 1}(\mathcal{N}, v) = \\ \sigma_{n}(\mathcal{N}_c, ..) \land \sigma_{n}(\mathcal{N}_w, ..) \land \sigma_{n}(\mathcal{N}_j, ..)$
  \end{enumerate}
\end{proof}

\begin{theorem}
  ${\bf R}(+, ;)$ is axiomatised with the axioms of join semilattice-ordered semigroups and $\{ \sigma_n \}_{n < \omega}$ defined in the proof of Lemma~\ref{ax}. Moreover, ${\bf R}(+, ;)$ has the decidable axiomatisation.
\end{theorem}
...
\begin{proof}
  ...
  In parallel, we check whether $\mathcal{A}$ has a finite representation. If there exists such $n < \omega$ such that $\mathcal{A}$ is representable over size $n$. Both sets are domains of corresponding computable functions, so they are enumerable. That makes ${\bf R}_{fin}(;, +)$ decidable by Post theorem.
\end{proof}
