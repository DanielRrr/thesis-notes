...
$ $
  ...
  \begin{enumerate}
    ...
    Let us put $\mathcal{N} = \bigcup \limits_{i < \omega} \mathcal{N}_i$.

    Clearly that every $l_{\mathcal{N}}(x, y)$ for each $x, y \in \operatorname{Nodes}(\mathcal{N})$ is join-prime. Let $a + b \in l_{\mathcal{N}}(x, y)$, then there exists $i < \omega$ with
    $a + b \in l_{\mathcal{N}_i}(x, y)$
    ...
    Let us ensure that the labelling function $l_{\mathcal{N}}$ respects $;$. Here, the argument is similar to the proof of \cite[Proposition 5]{hirsch2005class}.

    The map $rep$ defined as follows is a representation:
    \begin{center}
      $rep(a) = \{ (x, y) \: | \: a \in l_{\mathcal{N}}(x, y)\}$
    \end{center}
    This map is well-defined.

    $rep(a + b) = \{ (x, y) \: | \: a + b \in l_{\mathcal{N}}(x, y)\} \subseteq \{ (x, y) \: | \: a \in l_{\mathcal{N}}(x, y) \lor b \in l_{\mathcal{N}}(x, y) \} = rep(a) \cup rep(b)$.

    The converse inclusion:

    Suppose $(x,y) \in rep(a) \cup rep(b)$, so either $a \in l_{\mathcal{N}}(x,y)$ or $b \in l_{\mathcal{N}}(x,y)$. If $a \in l_{\mathcal{N}}(x,y)$, then $a + b \in l_{\mathcal{N}}(x,y)$ since $a \leq a + b$ and $l_{\mathcal{N}}(x,y)$ is an upset. The last holds because
    $l_{\mathcal{N}}(x,y) = \bigcup \limits_{i < \omega} l_{\mathcal{N}_i}(x,y)$ and the union of upsets is an upset.

    The second alternative ($b \in l_{\mathcal{N}}(x,y)$) is similar to the second one.

    Thus, $rep(a + b) = rep(a) \cup rep(b)$.

\medskip

Let us check that $rep(a ; b) = rep(a) | rep(b)$.

$rep(a;b) = \{ (x,y) \: | \: a;b \in l_{\mathcal{N}}(x,y) \}$

Suppose $(x,y) \in rep(a) ; rep(b)$, we need $(x, y) \in rep(a ; b)$, i.e. $a;b \in l_{\mathcal{N}}(x,y)$. Thus, there exists $z \in \operatorname{Nodes}(\mathcal{N})$ such that $(x,z) \in rep(a)$ and $(z,y) \in rep(b)$, i.e. $a \in l_{\mathcal{N}}(x,z)$ and $b \in l_{\mathcal{N}}(z, y)$. $\mathcal{N}$ is a network, so $l_{\mathcal{N}}(x,z) ; l_{\mathcal{N}}(z,y) \subseteq l_{\mathcal{N}}(x,y)$. Thus, $a ; b \in l_{\mathcal{N}}(x,y)$.

Suppose for the converse, $(x, y) \in rep(a ; b)$, i.e. $a;b \in l_{\mathcal{N}}(x,y)$. We need some $z$ such that $(x, z) \in rep(a)$ and $(z, y) \in rep(b)$, i.e. $a \in l_{\mathcal{N}}(x,z)$ and $b \in l_{\mathcal{N}}(z,y)$. (Ideally, we need some $c$ with $c \leq a ; b$, but $l_{\mathcal{N}}(x,y)$ is not downward directed)
  \end{enumerate}
\end{proof}
