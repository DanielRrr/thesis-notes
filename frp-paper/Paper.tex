...
We have already considered finite and countable algebras. Let us consider also arbitrary to complete the axiomatisation of ${\bf R}(;,+)$. We have the following statement.

\begin{lemma}
  Let $\mathcal{A}$ be a join-semilattice semigroup, then $\exists$ has a winning strategy has a winning strategy in $\mathcal{G}_m(\mathcal{A})$ iff $\mathcal{A}$ is elementary equivalent to some countable representable join-semilattice semigroups.
\end{lemma}

\begin{proof}
  The $\Leftarrow$ implication is obvious. If $\mathcal{A} \equiv \mathcal{A}'$, where $|\mathcal{A}'| = \omega$ and $\mathcal{A}' \in {\bf R}(;,+)$, then $\exists$ has a winning strategy in $\mathcal{G}_{\omega}(\mathcal{A}')$ (Lemma~\ref{rep}), so $\exists$ has a winning strategy in
  every $\mathcal{G}_{m}(\mathcal{A}')$ for $m < \omega$ (Proposition~\ref{fin}). Thus, so $\exists$ has a winning strategy in
  every $\mathcal{G}_{m}(\mathcal{A})$ since $\mathcal{A}$ and $\mathcal{A}'$ are elementary equivalent.

  Suppose $\exists$ has a winning strategy has a winning strategy in $\mathcal{G}_m(\mathcal{A})$ for $m < \omega$. The proof scheme is the same as for relation (see \cite[Lemma 19]{hirsch1997complete}) or cylindric algebras (see \cite[Corollary 3.3.5]{hirsch2013completions}), so we provide the following sketch. In particular, we have the following proof that may be proved using \strokeL o\'s's theorem (see \cite[Theorem 9.5.1]{hodges1993model}) similarly to.

  \begin{claim}
    If $\exists$ has a winning strategy has a winning strategy in $\mathcal{G}_m(\mathcal{A})$ for $m < \omega$, then $\exists$ has a winning strategy for $\mathcal{G}_{\omega}(\Pi_{U} \mathcal{A})$, where $\Pi_{U} \mathcal{A}$ is a non-principal ultrapower of $\mathcal{A}$ for $U$, an ultrafilter over $\omega$.
  \end{claim}

  Consider $\mathcal{B}$, a countable elementary equivalent subalgbera of $\Pi_{U} \mathcal{A}$ that exists by the downward L\"{o}wenheim-Skolem theorem, see \cite[Corollary 3.1.5]{hodges1993model}.
  We construct the following elementary chain inxeded over $\omega$:
  \begin{center}
    $\mathcal{B}_0 \preceq \mathcal{B}_1 \preceq \dots \mathcal{B}_i \preceq\mathcal{B}_{i + 1} \preceq \dots (\preceq \Pi_{U} \mathcal{A})$.
  \end{center}
  Let us put $\mathcal{B}_0 := \mathcal{B}$.
  Suppose the initial segment $\mathcal{B}_0 \preceq \mathcal{B}_1 \preceq \dots \mathcal{B}_i$ has been already constructed. Let us constuct $\mathcal{B}_{i + 1}$.

  Consider a function $\sigma$ that takes an initial game of the play $\mathcal{G}_{\omega}(\Pi_{U} \mathcal{A})$, say, $\mathcal{N}_0 \subseteq \mathcal{N}_1 \subseteq \dots \mathcal{N}_k$ (all $k$) and Abelard's move and returns $\exists$'s repsonse, $\mathcal{N}_{k + 1}$, an extension of $\mathcal{N}_k$.

  Consider $\sigma \upharpoonright_{\mathcal{B}_i}$, a restriction of $\sigma$, where $\forall$ takes elements only from $\mathcal{B}_i$ for his moves. The range of this restriction consists of $\Pi_{U} \mathcal{A}$-networks, and $E$, the set of elements that occur in those networks is also countable. We define $\mathcal{B}_{i + 1}$ as a countable elementary subalgebra of $\Pi_{U} \mathcal{A}$ that contains $\mathcal{B}_{i}$ and $E$. The existence follows from the downward L\"{o}wenheim-Skolem theorem.

  Let us put $\mathfrak{B} = \bigcup \limits_{i < \omega} \mathcal{B}_i$. Note that $\mathfrak{B}$ is countable. By Tarksi-Vaught elementary chain union theorem (see \cite[Theorem 2.5.2]{hodges1993model}), for each $i < \omega$ $\mathcal{B}_i \preceq \mathfrak{B}$. Therefore, $\mathfrak{B} \equiv \Pi_{U} \mathcal{A}$.
  On the other hand, $\mathcal{A} \equiv \Pi_{U} \mathcal{A}$ (see \cite[Corollary 9.5.2]{hodges1993model}).
  Moreover, $\mathfrak{B}$ is representable and $\mathfrak{B} \equiv \mathcal{A}$.
\end{proof}
