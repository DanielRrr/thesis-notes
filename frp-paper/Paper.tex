...
\begin{proof}
  ...
  Suppose $\exists$ has a winning strategy has a winning strategy in $\mathcal{G}_m(\mathcal{A})$ for $m < \omega$. The proof scheme is the same as for relation or cylindric algebras, so we provide the following sketch. In particular, we have the following proof that may be proved using \strokeL o\'s's Theorem similarly to.
  ...
  Consider $\mathcal{B}$, a countable elementary equivalent subalgbera of $\Pi_{U} \mathcal{A}$ that exists by the downward L\"{o}wenheim-Skolem theorem.
  ...
  Let us put $\mathfrak{B} = \bigcup \limits_{i < \omega} \mathcal{B}_i$. Note that $\mathfrak{B}$ is countable. By Tarksi-Vaught elementary chain union theorem, for each $i < \omega$ $\mathcal{B}_i \preceq \mathfrak{B}$. Therefore, $\mathfrak{B} \equiv \Pi_{U} \mathcal{A}$. On the other hand, $\mathcal{A} \equiv \Pi_{U} \mathcal{A}$.
