...
\begin{theorem}
  Let $\mathcal{M} = \langle W, R, \vartheta \rangle$ be an Euclidean model, $\Gamma$ a set of $\operatorname{Sub}$-closed formulas, and $\sim$ an equivalence relation having a finite index that respects $\Gamma$, then $\widehat{R} = (R^{min}_{\Gamma})^{E} \subseteq R^{max}_{\Gamma}$.

  Thus, ${\bf K}5$ admits strict filtrations.
\end{theorem}

\begin{proof}
  We put $R^{min}_{\Gamma} := \overline{R}$ for brevity.

  Recall that $\overline{R}^{E}$ has the form $\bigcup \limits_{n < \omega} (\overline{R})_n$, where
  \begin{enumerate}
    \item $\overline{R}_0 = \widehat{R}$
    \item $\overline{R}_{m + 1} = \overline{R}_n \cup ((\overline{R})^{-1} \circ \overline{R}_n)$
  \end{enumerate}
  One needs to show that for each $n < \omega$ $(\overline{R})_n \subseteq R^{max}_{\Gamma}$. We prove this by induction.
  Suppose $\mathcal{M}, y \models \varphi$ for $\Diamond \varphi \in \Phi$ and $\hat{x} \overline{R}^{E} \hat{x}$. We need $\mathcal{M}, x \models \Diamond \varphi$.
  \begin{enumerate}
    \item The case of $n = 0$ is trivial and it follows directly from the definition of a minimal filtration.
    \item Suppose $n = 1$. $\hat{x} \overline{R}_1 \hat{y}$ means that $(\hat{x}, \hat{y}) \in \overline{R} \cup (\overline{R}^{-1} \circ \overline{R})$.
    Let us assume that $(\hat{x}, \hat{y}) \in \overline{R}^{-1} \circ \overline{R}$. That is, there exists $\hat{z} \in W / \sim_{\Gamma}$ such that $(\hat{z}, \hat{x}), (\hat{z}, \hat{y}) \in \overline{R}$.

    $\hat{z} \overline{R} \hat{y}$ means that there are $z' \in \hat{z}$ and $y' \in \hat{y}$ such that $z' R y'$. $\mathcal{M}, y \models \varphi$ implies $\mathcal{M}, y' \models \varphi$.
    Then $\mathcal{M}, z' \models \Diamond \varphi$. $\hat{z} \overline{R} \hat{x}$ implies that there are $z'' \in \hat{z}$ and $x' \in \hat{x}$ with $z'' R x'$.

    $z'' \sim_{\Gamma} z'$ implies $\mathcal{M}, z'' \models \Diamond \varphi$. $\mathcal{M} \models {\bf K}5$, so $\mathcal{M}, z'' \models \Box \Diamond \varphi$. Thus, $\mathcal{M}, x' \models \Diamond \varphi$ since $x' \in R(z'')$. But $x' \in \hat{x}$, then $\mathcal{M}, x \models \Diamond \varphi$.

    \item Suppose $(\overline{R})_n \subseteq R^{max}_{\Gamma}$. Let us show that for $(\overline{R})_{n + 1}$. Then either $(\hat{x}, \hat{y}) \in (\overline{R})_n$ or $(\hat{z}, \hat{y}), (\hat{z}, \hat{x}) \in (\overline{R})_n$. The first alternative holds due to the IH. We consider only the second one.

    By Lemma..., we may visualise $(\hat{z}, \hat{y}), (\hat{z}, \hat{x}) \in (\overline{R})_n$ with the following sequences for some $\widehat{u_1}, \dots, \widehat{u_n}, \widehat{t_1}, \dots, \widehat{t_n}$:

    \xymatrix{
    &&& \hat{z} & \ar[l]_{\overline{R}} \widehat{u_1} \ar@{-}[r]^{R'} & \widehat{u_2} \ar@{-}[r]^{R'} & \dots \ar@{-}[r]^{R'} & \widehat{u_{n - 1}} \ar@{-}[r]^{R'} & \widehat{u_n} \ar[r]^{\overline{R}} & \hat{y} \\
    &&& \hat{z} & \ar[l]^{\overline{R}} \widehat{t_1} \ar@{-}[r]_{R'} & \widehat{t_2} \ar@{-}[r]_{R'} & \dots \ar@{-}[r]_{R'} & \widehat{t_{n - 1}} \ar@{-}[r]_{R'} & \widehat{t_n} \ar[r]_{\overline{R}} & \hat{x}
    }
    where $R'$ is either $\overline{R}$ or $\overline{R}^{-1}$

    $\mathcal{M}, y \models \varphi$ and $\widehat{u_n} \overline{R} \hat{y}$, so for some $u_n^{'} \in \widehat{u_n}$ and $y' \in \hat{y}$ with $u_n^{'} R y'$, so $\mathcal{M}, u_n^{'} \models \Diamond \varphi$ and $\widehat{\mathcal{M}}, \widehat{u_n} \models \Diamond \varphi$.

    We have either $\widehat{u_{n - 1}} \overline{R} \widehat{u_n}$ or $\widehat{u_n} \overline{R} \widehat{u_{n - 1}}$.

    If the first holds, then there are $u_{n - 1}^{'} \in \widehat{u_{n - 1}}$ and $u_{n}^{''} \in \widehat{u_{n}}$ with $u_{n - 1}^{'} R u_{n}^{''}$.
    $\mathcal{M}, u_{n}^{''} \models \Diamond \varphi$. wtf?

    If the second holds, then there $u_{n - 1}^{'} \in \widehat{u_{n - 1}}$ and $u_{n}^{''} \in \widehat{u_{n}}$ with $u_{n}^{''} R u_{n - 1}^{'}$. $\mathcal{M}, u_{n}^{''} \models \Diamond \varphi$ implies $\mathcal{M}, u_{n}^{''} \models \Box \Diamond \varphi$, so $\mathcal{M}, u_{n - 1}^{'} \models \Diamond \varphi$.
