...
The converse implication does not have to true generally.

Recall that ${\bf GL} = {\bf K} \oplus \Diamond (p \land \neg \Diamond p) \to \Diamond p$. ${\bf GL}$ does not admit filtrations, but one may prove that ${\bf GL}$ has the FMP using the selective filtation technique.

\begin{defin}
  Let $\mathcal{M} = \langle W, R, \vartheta \rangle$ be a model and $\Gamma$ a set closed under subformulas, then a weak submodel $\mathcal{M} = \langle W', R', \eta \rangle$ (that is, $W' \subseteq W$, $R' \subseteq R$, $\eta = \vartheta \upharpoonright_{W'}$) is called a selective filtration of $\mathcal{M}$ through $\Gamma$ is the following holds, for $\Diamond \psi \in \Gamma$, $x \in W'$:
  \begin{center}
    $\mathcal{M}, x \models \Diamond \psi$ iff $\mathcal{M}, y \models \psi$ for some $y \in R'(x)$
  \end{center}
\end{defin}

\begin{theorem}
  ${\bf GL}$ admits the ``finite canonical'' model property.
\end{theorem}

\begin{proof}
  Let $\Gamma$ be a finite $\operatorname{\bf Sub}$-closed set. So, $\Gamma = \operatorname{\bf Sub}(\gamma_1) \cup \dots \operatorname{\bf Sub}(\gamma_n)$, for some $\gamma_1, \dots, \gamma_n \in \Gamma$ and every $\gamma_i$ is ${\bf GL}$-consistent.


  $\gamma$ is true in $\mathcal{M}_{\Gamma} = \langle V, R^{\bf GL}, \vartheta \rangle$, where
  \begin{center}
  $V_{\Gamma} = \bigcup \limits_{\psi \in \Gamma} \{ x \in W_{\bf GL} \: | \: \mathcal{M}_{\bf GL}, x \models \psi \land \neg \Diamond \psi \}$
  \end{center}
  since $\mathcal{M}_{\Gamma}$ is a selective filtration of $\mathcal{M}_{\bf M}$ through $\Gamma$.

  Let $x \in V$, consider $||x|| = \{ \psi \in \Gamma \: | \: \mathcal{M}_{\bf GL}, x \models \psi \}$
  and define $x R^{\bf GL} y$ implies $||x|| R_{\Gamma} ||y||$. Every $||x||$ is clearly finite.
  The valuation map is usual, $\vartheta_{\Gamma}(p) = \{ ||x|| \: | \: p \in ||x|| \}$ for $p \in \operatorname{PV} \cap \Gamma$. We put $W_{\Gamma}$

  Let us check that $\langle W_{\Gamma}, R_{\Gamma}, \vartheta_{\Gamma} \rangle$ is a ``finite canonical`` ${\bf GL}$-model.
  Let us check that $R_{\Gamma}$ is transitive and Noetherian and it has no relflexive points.
  If $W_{\Gamma}$ had a relfexive point, say $||y||$, then we would have $\mathcal{M}, y \models \psi \to \Diamond \psi$, but that is not impossible. Transitivity is obvious.

  Clearly that $x R^{\bf GL} y$ implies $||x|| R_{\Gamma} ||y||$ and vice versa. So, if we had an infinite decreasing $R_{\Gamma}$-chain, then we would have an infinite decreasing $R^{\bf GL}$-chain, but that is not possible.
  So $\langle W_{\Gamma}, R_{\Gamma}, \vartheta_{\Gamma} \rangle$ is a ``finite canonical`` ${\bf GL}$-model.
\end{proof}

So we have an example of a logic that has the ``finite canonical'' model property with no filtrations.
